pipeline {
  ...
  environment {
    ...
    EMAIL_CONTENT =
        '''$PROJECT_NAME - GIT_BRANCH_PLACEHOLDER - $BUILD_STATUS! - Build # $BUILD_NUMBER:
           ...
           |
           |There were ${TEST_COUNTS,var="pass"} successful tests.
           |
           |There were ${TEST_COUNTS,var="fail"} failed tests.
           |
           |There were ${TEST_COUNTS,var="skip"} skipped tests.
           |
  ...
  stages {
    ...
    stage('test') {
      steps {
        sh '''
          pytest --cov-report xml:reports/coverage.xml --cov=src -v --junitxml=reports/results.xml -o junit_family=xunit1
        '''
      }
      post {
        always {
